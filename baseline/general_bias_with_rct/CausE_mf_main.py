...
from dataloader import YahooUniformImplicitBCELossDataLoader
from evaluate import ImplicitTestManager
from baseline_models import CausEMatrixFactorization
from baseline_train import CausETrainManager
...
DATASET_PATH = '/Coat_uniform_data/'
...
def main(
        ...
        train_config: dict,
        evaluate_config: dict,
        data_loader: YahooUniformImplicitBCELossDataLoader,
        random_seed: int,
        silent: bool = False,
        auto: bool = False,
        query: bool = True,
    ...
    model: CausEMatrixFactorization = CausEMatrixFactorization(
    ...
    uniform_tensor: torch.LongTensor = torch.LongTensor(data_loader.uniform_data_np).to(device)

    print(train_tensor.shape)
    assert train_tensor.shape[1] == 3
    model_config.update(
        {
            'n_user': data_loader.user_num,
            'n_item': data_loader.item_num,
        }
    )
    train_manager: CausETrainManager = CausETrainManager(
        model=model, evaluator=evaluator, training_data=train_tensor,
        device=device, uniform_data=uniform_tensor,
        ...
        test_begin_epoch=train_config['test_begin_epoch'], uniform_loss_coe=train_config['uniform_loss_coe'],
        teacher_reg_coe=train_config['teacher_reg_coe'], teacher_reg_mode=train_config['teacher_reg_mode'],
        teacher_L2_coe=train_config['teacher_L2_coe']
    )
...
if __name__ == '__main__':
    loader: YahooUniformImplicitBCELossDataLoader = YahooUniformImplicitBCELossDataLoader(
